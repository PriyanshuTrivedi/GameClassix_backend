...
let multiSolutionExits,solnExits,totalToRemove;
...
const cnvrt2DArrToString=(arr2D)=>{
    let ans,i,j;
    ans="";
    for(i=0;i<n;i++){
        for(j=0;j<n;j++){
...
const Sudoku_RandomSolvableSudokuWithSolution=(difficulty)=>{
    try{
        let blanksAccToDifficulty=53;
        switch(difficulty){
            case "beginner":
                blanksAccToDifficulty=31+Math.floor(8*Math.random());
                break;
            case "easy":
                blanksAccToDifficulty=39+Math.floor(8*Math.random());
                break;
            case "medium":
                blanksAccToDifficulty=47+Math.floor(4*Math.random());
                break;
            case "hard":
                blanksAccToDifficulty=51+Math.floor(3*Math.random());
                break;
        }
        totalToRemove=blanksAccToDifficulty;
        generateUniqueSolvableSudokuWith50Blanks();
        const completeSudokuStr=cnvrt2DArrToString(completeSudoku);
        const partiallyFilledSudokuWithUniqueSolutionStr=cnvrt2DArrToString(uniqueSolvableSudoku);
        let FinalResult={
            completeSudoku:completeSudokuStr,
            partiallyFilledSudokuWithUniqueSolution:partiallyFilledSudokuWithUniqueSolutionStr,
            blanks:blanksAccToDifficulty
        }
        return FinalResult;
    }catch(e)
    {
...
module.exports={
    Sudoku_RandomSolvableSudokuWithSolution,
};